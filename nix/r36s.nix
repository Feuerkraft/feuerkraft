...
#   nix build .#feuerkraft-r36s
#   nix build .#feuerkraft-r36s-portmaster   # PortMaster tree for /roms/ports
#
{ lib
, stdenv
, stdenvNoCC
...
let
  ...
  , enableSound ? true
    ...
    stdenv.mkDerivation {
      ...
      postInstall = ''
        mkdir -p $out/share/feuerkraft
        # CMake may have installed data/ as non-writable; ensure we can add files.
        chmod -R u+w $out/share/feuerkraft || true
        if [ -d "$src/data" ]; then
          cp -a "$src/data/." $out/share/feuerkraft/ || true
          chmod -R u+w $out/share/feuerkraft || true
        fi
        cat > $out/share/feuerkraft/README-R36S.txt << EOF_README
Feuerkraft — R36S / ArkOS (sysroot-linked)
====================================================

Binary: bin/feuerkraft
  SDL2 + GLES2, linked against the ArkOS aarch64 sysroot.

Deploy the binary + share/feuerkraft data to the device.
...
often joystick-only unless SDL_GAMECONTROLLERCONFIG is set.
  ...
  Preferred: nix build .#feuerkraft-r36s-portmaster
...
EOF_README
        cat > $out/share/feuerkraft/feuerkraft.sh << 'LAUNCH'
#!/bin/bash
DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$DIR"
BIN="$DIR/../bin/feuerkraft"
if [ ! -x "$BIN" ]; then BIN="$DIR/feuerkraft"; fi
...
exec "$BIN" --fullscreen "$@"
LAUNCH
        chmod +x $out/share/feuerkraft/feuerkraft.sh
      '';

      meta = with lib; {
        description = "Feuerkraft for R36S/ArkOS (sysroot-linked aarch64)";
  ...
  # Wrap a mkFeuerkraftR36s result as a PortMaster-ready tree:
  #   Feuerkraft.sh + feuerkraft/ + metadata
  # Copy into /roms/ports/ or zip for PortMaster autoinstall.
  mkFeuerkraftR36sPortMaster = {
    r36sPkg
  , version
  ...
  , screenshotSrc ? null
    ...
    stdenvNoCC.mkDerivation {
      ...
      installPhase = ''
        ...
        install -m755 "${r36sPkg}/bin/feuerkraft" \
          "$gamedir/feuerkraft"

        # Game data (CMake DATA_PREFIX was share/feuerkraft).
        # Store paths are mode 444/555; make writable so we can drop helpers.
        if [ -d "${r36sPkg}/share/feuerkraft" ]; then
          cp -a "${r36sPkg}/share/feuerkraft/." "$gamedir/data/"
          chmod -R u+w "$gamedir/data"
          # Keep only game assets under data/; drop packaging helpers
          rm -f "$gamedir/data/feuerkraft.sh" \
...
Feuerkraft — see upstream GPL-3.0-or-later and LICENSES/ in the source tree.
...
#!/bin/bash
# Feuerkraft — PortMaster launcher for R36S / ArkOS
...
GAMEDIR="/$directory/ports/feuerkraft"
...
pm_platform_helper "$GAMEDIR/feuerkraft" 2>/dev/null || true
        ...
        cat > "$root/port.json" << EOF_JSON
{
  ...
  "attr": {
    "title": "${title}",
    "desc": "Classic Feuerkraft (SDL2 + GLES2) for ArkOS / R36S. Free jump-and-run platformer starring Tux.",
    "inst": "Ready to run. Copy Feuerkraft.sh and the feuerkraft/ folder into /roms/ports/ (or install the zip via PortMaster autoinstall).",
    "genres": ["platform", "action"],
    "porter": ["Feuerkraft-Origins"],
    "image": {},
    "rtr": true,
    "runtime": null,
    "reqs": [],
...
<gameList>
  <game>
    ...
    <name>${title}</name>
    <desc>Classic Feuerkraft — free jump-and-run platformer starring Tux. SDL2 + GLES2 build for ArkOS / R36S.</desc>
    <releasedate>20040511T000000</releasedate>
    <developer>Feuerkraft Team</developer>
    <publisher>Feuerkraft-Origins</publisher>
...
## Feuerkraft (R36S / ArkOS)

Native **aarch64** build linked against the ArkOS sysroot (SDL2 + GLES2).

### Install

1. Copy `Feuerkraft.sh` and the `feuerkraft/` directory to `/roms/ports/` on the device, **or**
...
Thanks to the Feuerkraft developers and the PortMaster / ArkOS communities.
EOF_README

        '';

      meta = with lib; {
        description = "PortMaster package of Feuerkraft for R36S/ArkOS";
        license = licenses.gpl3Plus;
        platforms = platforms.linux;
        hydraPlatforms = [];
      };
  ...
  mkFeuerkraftR36sPortMasterZip = {
    portMasterPkg
  , version
  , pname ? "feuerkraft-r36s-portmaster-zip"
  , zipName ? "feuerkraft.zip"
  }:
    stdenvNoCC.mkDerivation {
      inherit pname version;
      dontUnpack = true;
      ...
      meta = with lib; {
        description = "PortMaster autoinstall zip of Feuerkraft for R36S/ArkOS";
        license = licenses.gpl3Plus;
        platforms = platforms.linux;
        hydraPlatforms = [];
      };
...
in
{
  inherit arkosSysroot mkFeuerkraftR36s mkFeuerkraftR36sPortMaster mkFeuerkraftR36sPortMasterZip;
}
