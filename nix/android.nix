...
let
  ...
  mkApk = {
    ...
    outApkName,
    keystore,
    # Feuerkraft (and similar): C++ sources live outside appDir.
    gameSrcDir ? null,
    gameRootDir ? null,
    ...
    gameDataDir ? null,
    stbImageH ? null,
    # Full version for FEUERKRAFT_VERSION (e.g. 0.1.5-dev+gabc1234).
    ...
    pkgs.stdenvNoCC.mkDerivation {
      ...
      env = {
        ...
        KEYSTORE = "${keystore}";
        FEUERKRAFT_VERSION = gameVersion;
      } // pkgs.lib.optionalAttrs (gameSrcDir != null) {
        GAME_SRC_DIR = "${gameSrcDir}";
      } // pkgs.lib.optionalAttrs (gameRootDir != null) {
        GAME_ROOT_DIR = "${gameRootDir}";
      } // pkgs.lib.optionalAttrs (sdl2ImageSrc != null) {
        SDL2_IMAGE_SRC = "${sdl2ImageSrc}";
      } // (
        if gameDataDir == null then
          throw "mkApk: gameDataDir is required for Feuerkraft Android APK"
        else {
