...
void
CommunicationDialog::draw (SDL_Renderer* gc)
{
      ...
      if (unit)
        {
          ...
          auto bbox = font.bounding_rect(0, 0, i->second.text);
          ...
          int x1 = int(dialog_pos.x - bbox.get_width()/2 - 10);
          int y1 = int(dialog_pos.y - 10);
          Display::fill_rect(x1, y1,
                             x1 + bbox.get_width() + 20,
                             y1 + bbox.get_height() + 20,
                             Color(0,0,0, 100));
