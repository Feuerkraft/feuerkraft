# Cross-compiling Feuerkraft for R36S (ArkOS)
...
Copy `build-r36s/feuerkraft` and the `data/` tree to the device (e.g. under `/roms/ports/feuerkraft/`).
...
nix build .#feuerkraft-r36s   # links against that sysroot
```
...
# → result/bin/feuerkraft (aarch64, SDL2+GLES2, ArkOS sysroot)

# PortMaster-ready tree (launcher + data + metadata):
nix build .#feuerkraft-r36s-portmaster
# → result/Feuerkraft.sh , result/feuerkraft/ , …
#    cp -a result/* /roms/ports/

# PortMaster autoinstall zip:
nix build .#feuerkraft-r36s-portmaster-zip
# → result/feuerkraft.zip  →  ports/PortMaster/autoinstall/
```
...
```bash
nix build .#feuerkraft-r36s-portmaster
cp -a result/"Feuerkraft.sh" result/feuerkraft /roms/ports/

# Or autoinstall zip:
nix build .#feuerkraft-r36s-portmaster-zip
# copy result/feuerkraft.zip → ports/PortMaster/autoinstall/
```
...
```text
/roms/ports/
  Feuerkraft.sh      # launcher (must source PortMaster control.txt)
  feuerkraft/
    feuerkraft        # binary
    data/                      # game data
```
