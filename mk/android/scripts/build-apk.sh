#!/usr/bin/env bash
# Builds the Feuerkraft APK, linking SDL2 as a prebuilt library.
...
#   FEUERKRAFT_VERSION           - full version string (e.g. 0.1.5-dev+gabc1234)
set -euo pipefail
...
# Vendored s7 (referenced as ../s7 from jni/src/Android.mk).
if [ -n "${GAME_ROOT_DIR:-}" ] && [ -d "$GAME_ROOT_DIR/external/s7" ]; then
  mkdir -p src/jni/s7
  cp -a "$GAME_ROOT_DIR/external/s7/s7.c" "$GAME_ROOT_DIR/external/s7/s7.h" src/jni/s7/
elif [ -d "$GAME_SRC_DIR/../external/s7" ]; then
  mkdir -p src/jni/s7
  cp -a "$GAME_SRC_DIR/../external/s7/s7.c" "$GAME_SRC_DIR/../external/s7/s7.h" src/jni/s7/
else
  echo "error: need external/s7 (set GAME_ROOT_DIR to repo root)" >&2
  exit 1
fi
...
# Bake VERSION+g<rev> into FEUERKRAFT_VERSION (see jni/Android.mk).
FEUERKRAFT_VERSION="${FEUERKRAFT_VERSION:-0.1.5-dev}"
echo "==> FEUERKRAFT_VERSION=$FEUERKRAFT_VERSION"

"$NDK/ndk-build" \
  NDK_PROJECT_PATH="$PWD/src" \
  APP_BUILD_SCRIPT="$PWD/src/jni/Android.mk" \
  NDK_APPLICATION_MK="$PWD/src/jni/Application.mk" \
  FEUERKRAFT_VERSION="$FEUERKRAFT_VERSION" \
