#!/usr/bin/env bash
# Cross-compiles Feuerkraft to wasm32 + HTML via emcmake.
# Expects (set by nix/wasm.nix):
#   APP_NAME         - binary/html basename (e.g. feuerkraft)
...
#   PROJECT_VERSION_FULL - e.g. 0.1.5-dev+gabc1234 (CMake FEUERKRAFT_VERSION)
...
LINK_FLAGS=(
  # Fixed heap: Chrome/WebGL reject ArrayBufferView backed by a *resizable*
  # buffer (WASM memory with ALLOW_MEMORY_GROWTH). Feuerkraft M1 needs only
  # modest RAM (data ~18MB + textures); 128MB is ample and keeps the buffer
  # non-resizable so TextDecoder + texImage2D work without copies.
  ...
  "SHELL:-sEXPORTED_FUNCTIONS=_main,_fk_emscripten_canvas_resize,_fk_emscripten_canvas_native,_emscripten_pause_main_loop,_emscripten_resume_main_loop,_fk_emscripten_audio_pause,_fk_emscripten_audio_resume"
...
# Work around TextDecoder rejecting resizable ArrayBuffers (WASM memory with
# ALLOW_MEMORY_GROWTH). Newer browsers throw:
#   Failed to execute 'decode' on 'TextDecoder': The provided ArrayBuffer value must not be resizable
# in UTF8ArrayToString / openat. Emscripten 6+ no longer allows TEXTDECODER=0,
# so patch TextDecoder.decode via --pre-js to copy into a fixed buffer.
TEXTDECODER_FIX="${SRC_DIR}/mk/wasm/js/textdecoder-resizable-fix.js"
if [ -f "$TEXTDECODER_FIX" ]; then
  LINK_FLAGS+=("--pre-js" "$TEXTDECODER_FIX")
  echo "==> TextDecoder resizable-buffer fix: $TEXTDECODER_FIX"
fi
...
# Emscripten names the outputs after the CMake target (feuerkraft).
...
# Cache-bust the main script tag (locateFile covers .wasm/.data loaded from JS).
# {{{ SCRIPT }}} is expanded by emcc after our shell template, so the src has no query.
cache_v="${GIT_REV:-${PROJECT_VERSION_FULL:-dev}}"
if [ -f "${APP_NAME}.html" ] && [ -n "$cache_v" ]; then
  # Only rewrite the primary app .js reference; leave other scripts alone.
  sed -i "s|src=\"${APP_NAME}\\.js\"|src=\"${APP_NAME}.js?v=${cache_v}\"|g" "${APP_NAME}.html" || true
  echo "==> cache-bust script src with ?v=${cache_v}"
fi
